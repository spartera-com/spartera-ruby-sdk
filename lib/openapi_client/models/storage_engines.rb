...
module OpenapiClient
  ...
  class StorageEngines < ApiModelBase
    # Optional.
    ...
    # Optional.
    attr_accessor :integration_complete

    # Optional.
    attr_accessor :test_func_complete
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::StorageEngines` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::StorageEngines`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def engine_type=(engine_type)
      validator = EnumAttributeValidator.new('String', ["EDW", "LLM", "RDBMS", "OBJ", "API_MODEL", "EXTERNAL_API"])
