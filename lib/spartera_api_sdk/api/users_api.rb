=begin
#Spartera API Documentation

#Auto-generated API documentation for REST services of the Spartera platform

The version of the OpenAPI document: 1.24.0
Contact: support@spartera.com
Generated by: https://openapi-generator.tech
Generator version: 7.13.0

=end

require 'cgi'

module SparteraApiSdk
  class UsersApi
    attr_accessor :api_client

    def initialize(api_client = ApiClient.default)
      @api_client = api_client
    end
    # Get a list of all users in a company
    # @param company_id [String] 
    # @param [Hash] opts the optional parameters
    # @return [CompaniesCompanyIdUsersGet200Response]
    def companies_company_id_users_get(company_id, opts = {})
      data, _status_code, _headers = companies_company_id_users_get_with_http_info(company_id, opts)
      data
    end

    # Get a list of all users in a company
    # @param company_id [String] 
    # @param [Hash] opts the optional parameters
    # @return [Array<(CompaniesCompanyIdUsersGet200Response, Integer, Hash)>] CompaniesCompanyIdUsersGet200Response data, response status code and response headers
    def companies_company_id_users_get_with_http_info(company_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: UsersApi.companies_company_id_users_get ...'
      end
      # verify the required parameter 'company_id' is set
      if @api_client.config.client_side_validation && company_id.nil?
        fail ArgumentError, "Missing the required parameter 'company_id' when calling UsersApi.companies_company_id_users_get"
      end
      # resource path
      local_var_path = '/companies/{company_id}/users'.sub('{' + 'company_id' + '}', CGI.escape(company_id.to_s))

      # query parameters
      query_params = opts[:query_params] || {}

      # header parameters
      header_params = opts[:header_params] || {}
      # HTTP header 'Accept' (if needed)
      header_params['Accept'] = @api_client.select_header_accept(['application/json']) unless header_params['Accept']

      # form parameters
      form_params = opts[:form_params] || {}

      # http body (model)
      post_body = opts[:debug_body]

      # return_type
      return_type = opts[:debug_return_type] || 'CompaniesCompanyIdUsersGet200Response'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['ApiKeyAuth']

      new_options = opts.merge(
        :operation => :"UsersApi.companies_company_id_users_get",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
        :auth_names => auth_names,
        :return_type => return_type
      )

      data, status_code, headers = @api_client.call_api(:GET, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: UsersApi#companies_company_id_users_get\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end

    # Create a new user
    # @param company_id [String] 
    # @param users_input [UsersInput] 
    # @param [Hash] opts the optional parameters
    # @return [CompaniesCompanyIdUsersPost200Response]
    def companies_company_id_users_post(company_id, users_input, opts = {})
      data, _status_code, _headers = companies_company_id_users_post_with_http_info(company_id, users_input, opts)
      data
    end

    # Create a new user
    # @param company_id [String] 
    # @param users_input [UsersInput] 
    # @param [Hash] opts the optional parameters
    # @return [Array<(CompaniesCompanyIdUsersPost200Response, Integer, Hash)>] CompaniesCompanyIdUsersPost200Response data, response status code and response headers
    def companies_company_id_users_post_with_http_info(company_id, users_input, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: UsersApi.companies_company_id_users_post ...'
      end
      # verify the required parameter 'company_id' is set
      if @api_client.config.client_side_validation && company_id.nil?
        fail ArgumentError, "Missing the required parameter 'company_id' when calling UsersApi.companies_company_id_users_post"
      end
      # verify the required parameter 'users_input' is set
      if @api_client.config.client_side_validation && users_input.nil?
        fail ArgumentError, "Missing the required parameter 'users_input' when calling UsersApi.companies_company_id_users_post"
      end
      # resource path
      local_var_path = '/companies/{company_id}/users'.sub('{' + 'company_id' + '}', CGI.escape(company_id.to_s))

      # query parameters
      query_params = opts[:query_params] || {}

      # header parameters
      header_params = opts[:header_params] || {}
      # HTTP header 'Accept' (if needed)
      header_params['Accept'] = @api_client.select_header_accept(['application/json']) unless header_params['Accept']
      # HTTP header 'Content-Type'
      content_type = @api_client.select_header_content_type(['application/json'])
      if !content_type.nil?
          header_params['Content-Type'] = content_type
      end

      # form parameters
      form_params = opts[:form_params] || {}

      # http body (model)
      post_body = opts[:debug_body] || @api_client.object_to_http_body(users_input)

      # return_type
      return_type = opts[:debug_return_type] || 'CompaniesCompanyIdUsersPost200Response'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['ApiKeyAuth']

      new_options = opts.merge(
        :operation => :"UsersApi.companies_company_id_users_post",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
        :auth_names => auth_names,
        :return_type => return_type
      )

      data, status_code, headers = @api_client.call_api(:POST, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: UsersApi#companies_company_id_users_post\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end

    # Delete single user by ID
    # @param company_id [String] 
    # @param user_id [String] 
    # @param [Hash] opts the optional parameters
    # @return [CompaniesCompanyIdUsersUserIdDelete200Response]
    def companies_company_id_users_user_id_delete(company_id, user_id, opts = {})
      data, _status_code, _headers = companies_company_id_users_user_id_delete_with_http_info(company_id, user_id, opts)
      data
    end

    # Delete single user by ID
    # @param company_id [String] 
    # @param user_id [String] 
    # @param [Hash] opts the optional parameters
    # @return [Array<(CompaniesCompanyIdUsersUserIdDelete200Response, Integer, Hash)>] CompaniesCompanyIdUsersUserIdDelete200Response data, response status code and response headers
    def companies_company_id_users_user_id_delete_with_http_info(company_id, user_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: UsersApi.companies_company_id_users_user_id_delete ...'
      end
      # verify the required parameter 'company_id' is set
      if @api_client.config.client_side_validation && company_id.nil?
        fail ArgumentError, "Missing the required parameter 'company_id' when calling UsersApi.companies_company_id_users_user_id_delete"
      end
      # verify the required parameter 'user_id' is set
      if @api_client.config.client_side_validation && user_id.nil?
        fail ArgumentError, "Missing the required parameter 'user_id' when calling UsersApi.companies_company_id_users_user_id_delete"
      end
      # resource path
      local_var_path = '/companies/{company_id}/users/{user_id}'.sub('{' + 'company_id' + '}', CGI.escape(company_id.to_s)).sub('{' + 'user_id' + '}', CGI.escape(user_id.to_s))

      # query parameters
      query_params = opts[:query_params] || {}

      # header parameters
      header_params = opts[:header_params] || {}
      # HTTP header 'Accept' (if needed)
      header_params['Accept'] = @api_client.select_header_accept(['application/json']) unless header_params['Accept']

      # form parameters
      form_params = opts[:form_params] || {}

      # http body (model)
      post_body = opts[:debug_body]

      # return_type
      return_type = opts[:debug_return_type] || 'CompaniesCompanyIdUsersUserIdDelete200Response'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['ApiKeyAuth']

      new_options = opts.merge(
        :operation => :"UsersApi.companies_company_id_users_user_id_delete",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
        :auth_names => auth_names,
        :return_type => return_type
      )

      data, status_code, headers = @api_client.call_api(:DELETE, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: UsersApi#companies_company_id_users_user_id_delete\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end

    # Get single user by ID
    # @param company_id [String] 
    # @param user_id [String] 
    # @param [Hash] opts the optional parameters
    # @return [CompaniesCompanyIdUsersUserIdGet200Response]
    def companies_company_id_users_user_id_get(company_id, user_id, opts = {})
      data, _status_code, _headers = companies_company_id_users_user_id_get_with_http_info(company_id, user_id, opts)
      data
    end

    # Get single user by ID
    # @param company_id [String] 
    # @param user_id [String] 
    # @param [Hash] opts the optional parameters
    # @return [Array<(CompaniesCompanyIdUsersUserIdGet200Response, Integer, Hash)>] CompaniesCompanyIdUsersUserIdGet200Response data, response status code and response headers
    def companies_company_id_users_user_id_get_with_http_info(company_id, user_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: UsersApi.companies_company_id_users_user_id_get ...'
      end
      # verify the required parameter 'company_id' is set
      if @api_client.config.client_side_validation && company_id.nil?
        fail ArgumentError, "Missing the required parameter 'company_id' when calling UsersApi.companies_company_id_users_user_id_get"
      end
      # verify the required parameter 'user_id' is set
      if @api_client.config.client_side_validation && user_id.nil?
        fail ArgumentError, "Missing the required parameter 'user_id' when calling UsersApi.companies_company_id_users_user_id_get"
      end
      # resource path
      local_var_path = '/companies/{company_id}/users/{user_id}'.sub('{' + 'company_id' + '}', CGI.escape(company_id.to_s)).sub('{' + 'user_id' + '}', CGI.escape(user_id.to_s))

      # query parameters
      query_params = opts[:query_params] || {}

      # header parameters
      header_params = opts[:header_params] || {}
      # HTTP header 'Accept' (if needed)
      header_params['Accept'] = @api_client.select_header_accept(['application/json']) unless header_params['Accept']

      # form parameters
      form_params = opts[:form_params] || {}

      # http body (model)
      post_body = opts[:debug_body]

      # return_type
      return_type = opts[:debug_return_type] || 'CompaniesCompanyIdUsersUserIdGet200Response'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['ApiKeyAuth']

      new_options = opts.merge(
        :operation => :"UsersApi.companies_company_id_users_user_id_get",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
        :auth_names => auth_names,
        :return_type => return_type
      )

      data, status_code, headers = @api_client.call_api(:GET, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: UsersApi#companies_company_id_users_user_id_get\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end

    # Update an existing user by ID
    # @param company_id [String] 
    # @param user_id [String] 
    # @param users_update [UsersUpdate] 
    # @param [Hash] opts the optional parameters
    # @return [CompaniesCompanyIdUsersUserIdPatch200Response]
    def companies_company_id_users_user_id_patch(company_id, user_id, users_update, opts = {})
      data, _status_code, _headers = companies_company_id_users_user_id_patch_with_http_info(company_id, user_id, users_update, opts)
      data
    end

    # Update an existing user by ID
    # @param company_id [String] 
    # @param user_id [String] 
    # @param users_update [UsersUpdate] 
    # @param [Hash] opts the optional parameters
    # @return [Array<(CompaniesCompanyIdUsersUserIdPatch200Response, Integer, Hash)>] CompaniesCompanyIdUsersUserIdPatch200Response data, response status code and response headers
    def companies_company_id_users_user_id_patch_with_http_info(company_id, user_id, users_update, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: UsersApi.companies_company_id_users_user_id_patch ...'
      end
      # verify the required parameter 'company_id' is set
      if @api_client.config.client_side_validation && company_id.nil?
        fail ArgumentError, "Missing the required parameter 'company_id' when calling UsersApi.companies_company_id_users_user_id_patch"
      end
      # verify the required parameter 'user_id' is set
      if @api_client.config.client_side_validation && user_id.nil?
        fail ArgumentError, "Missing the required parameter 'user_id' when calling UsersApi.companies_company_id_users_user_id_patch"
      end
      # verify the required parameter 'users_update' is set
      if @api_client.config.client_side_validation && users_update.nil?
        fail ArgumentError, "Missing the required parameter 'users_update' when calling UsersApi.companies_company_id_users_user_id_patch"
      end
      # resource path
      local_var_path = '/companies/{company_id}/users/{user_id}'.sub('{' + 'company_id' + '}', CGI.escape(company_id.to_s)).sub('{' + 'user_id' + '}', CGI.escape(user_id.to_s))

      # query parameters
      query_params = opts[:query_params] || {}

      # header parameters
      header_params = opts[:header_params] || {}
      # HTTP header 'Accept' (if needed)
      header_params['Accept'] = @api_client.select_header_accept(['application/json']) unless header_params['Accept']
      # HTTP header 'Content-Type'
      content_type = @api_client.select_header_content_type(['application/json'])
      if !content_type.nil?
          header_params['Content-Type'] = content_type
      end

      # form parameters
      form_params = opts[:form_params] || {}

      # http body (model)
      post_body = opts[:debug_body] || @api_client.object_to_http_body(users_update)

      # return_type
      return_type = opts[:debug_return_type] || 'CompaniesCompanyIdUsersUserIdPatch200Response'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['ApiKeyAuth']

      new_options = opts.merge(
        :operation => :"UsersApi.companies_company_id_users_user_id_patch",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
        :auth_names => auth_names,
        :return_type => return_type
      )

      data, status_code, headers = @api_client.call_api(:PATCH, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: UsersApi#companies_company_id_users_user_id_patch\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end

    # Get current authenticated user's profile.
    # @param [Hash] opts the optional parameters
    # @return [CompaniesCompanyIdUsersGet200Response]
    def me_get(opts = {})
      data, _status_code, _headers = me_get_with_http_info(opts)
      data
    end

    # Get current authenticated user&#39;s profile.
    # @param [Hash] opts the optional parameters
    # @return [Array<(CompaniesCompanyIdUsersGet200Response, Integer, Hash)>] CompaniesCompanyIdUsersGet200Response data, response status code and response headers
    def me_get_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: UsersApi.me_get ...'
      end
      # resource path
      local_var_path = '/me'

      # query parameters
      query_params = opts[:query_params] || {}

      # header parameters
      header_params = opts[:header_params] || {}
      # HTTP header 'Accept' (if needed)
      header_params['Accept'] = @api_client.select_header_accept(['application/json']) unless header_params['Accept']

      # form parameters
      form_params = opts[:form_params] || {}

      # http body (model)
      post_body = opts[:debug_body]

      # return_type
      return_type = opts[:debug_return_type] || 'CompaniesCompanyIdUsersGet200Response'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['ApiKeyAuth']

      new_options = opts.merge(
        :operation => :"UsersApi.me_get",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
        :auth_names => auth_names,
        :return_type => return_type
      )

      data, status_code, headers = @api_client.call_api(:GET, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: UsersApi#me_get\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
  end
end
